# config_ablation5.py
# Configuration for Ablation 5: Production with Fast Motion Force Jetson
# - Same as Ablation 4 BUT forces medium tier and offload when motion is FAST
# - Tests new real-time policy for fast motion (safety > energy)
...
# ABLATION 5: Enable sensor fusion
...
# ABLATION 5: Multi-model consensus with nano models
model_size = "nano"
model_number = 3

# Use baseline models (trained on all 3 datasets) for single-model ablations
# Keep False for multi-model ablations to use existing best1.pt, best2.pt, best3.pt
USE_BASELINE_MODELS = False
...
# ABLATION 5: Enable baselines for sensor fusion
...
# ABLATION 5: Enable remote inference for higher states (production config)
INFERENCE_ROUTING = {
    "S0": "local",
    "S1": "remote",
    "S2": "remote",
...
# ABLATION 5: Force offload when motion is FAST
ALWAYS_OFFLOAD_ON_FAST_MOTION = True

# ABLATION 5: Force medium tier when motion is FAST (new config flag)
FAST_MOTION_TIER = "medium"

# ABLATION 5: Platform-specific tier (nano on Pi, small on Jetson)
...
RESULTS_STORAGE_DIR = "storage/data_results/ablation5"
RESULTS_VIDEO_STORAGE_DIR = "storage/video_results/ablation5"
