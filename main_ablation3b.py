"""
Main entry point for Ablation 3b: Single-Model Full System Local-Only
- FSM enabled, single nano model, sensor fusion enabled, all local
- Tests full system with single model vs Ablation 3 (3x nano)
"""

# Override config module BEFORE any other imports
import config_ablation3b as config
import sys
sys.modules['config'] = config

# Now import the rest (they will use config_ablation3b)
...
def main():
    """
    Main method for Ablation 3b.
    Initializes the MQTT receiver and directs each incoming message payload
    to the process_message callback for processing.
    """
    print("=" * 60)
    print("ABLATION 3b: Single-Model Full System Local-Only")
    print("=" * 60)
    
    # Extract ablation name from config file name (e.g., "config_ablation3b.py" -> "ablation3b")
    ...
    if 'ablation' in config_file:
        ...
        results_dir = getattr(config, 'RESULTS_STORAGE_DIR', 'storage/data_results/ablation3b')
        if '/ablation' in results_dir:
            ablation_name = results_dir.split('/ablation')[1].split('/')[0]
            ablation_name = f"ablation{ablation_name}"
        else:
            ablation_name = "ablation3b"  # default fallback
    ...
    # Structure: storage/image-detections/ablation3b/run_<timestamp>/
    ...
    print("  - Remote Offload: Disabled (all local)")
