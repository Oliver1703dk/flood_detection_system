...
import config_ablation1 as config
import sys
sys.modules['config'] = config

# Now import the rest (they will use config_ablation1)
...
def main():
    ...
    # Extract ablation name from config file name (e.g., "config_ablation1.py" -> "ablation1")
    ...
    if 'ablation' in config_file:
        ...
        results_dir = getattr(config, 'RESULTS_STORAGE_DIR', 'storage/data_results/ablation1')
        if '/ablation' in results_dir:
            ablation_name = results_dir.split('/ablation')[1].split('/')[0]
            ablation_name = f"ablation{ablation_name}"
        else:
            ablation_name = "ablation1"  # default fallback
    ...
    # Structure: storage/image-detections/ablation1/run_<timestamp>/
