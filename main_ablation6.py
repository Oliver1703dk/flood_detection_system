"""
Main entry point for Ablation 6: Always Offload Medium (Naive Baseline)
- Fixed medium tier, always offload to Jetson, no FSM adaptive logic
"""

# Override config module BEFORE any other imports
import config_ablation6 as config
import sys
sys.modules['config'] = config

# Now import the rest (they will use config_ablation6)
...
def main():
    ...
    """
    print("=" * 60)
    print("ABLATION 6: Always Offload Medium (Naive Baseline)")
    print("=" * 60)
    
    # Extract ablation name from config file name (e.g., "config_ablation6.py" -> "ablation6")
    ...
    if 'ablation' in config_file:
        ...
        results_dir = getattr(config, 'RESULTS_STORAGE_DIR', 'storage/data_results/ablation6')
        if '/ablation' in results_dir:
            ablation_name = results_dir.split('/ablation')[1].split('/')[0]
            ablation_name = f"ablation{ablation_name}"
        else:
            ablation_name = "ablation6"  # default fallback
    ...
    # Structure: storage/image-detections/ablation6/run_<timestamp>/
