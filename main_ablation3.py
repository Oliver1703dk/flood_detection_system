...
import config_ablation3 as config
import sys
sys.modules['config'] = config

# Now import the rest (they will use config_ablation3)
...
def main():
    ...
    # Extract ablation name from config file name (e.g., "config_ablation3.py" -> "ablation3")
    ...
    if 'ablation' in config_file:
        ...
        results_dir = getattr(config, 'RESULTS_STORAGE_DIR', 'storage/data_results/ablation3')
        if '/ablation' in results_dir:
            ablation_name = results_dir.split('/ablation')[1].split('/')[0]
            ablation_name = f"ablation{ablation_name}"
        else:
            ablation_name = "ablation3"  # default fallback
    ...
    # Structure: storage/image-detections/ablation3/run_<timestamp>/
